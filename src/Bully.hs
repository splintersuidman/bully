...
import Text.Pandoc.Extensions qualified as Pandoc
import Text.Pandoc.Format qualified as Pandoc
import Text.Pandoc.PDF qualified as Pandoc
import Text.Pandoc.Readers qualified as Pandoc
...
mapContribution' f contribution =
  contribution
...
instance ToMetaValue a => ToMetaValue (Contribution a) where
  toMetaValue contribution =
    toMetaValue @(Map Text MetaValue) $
      Map.fromList
...
formatFromOutput filename = \case
  Nothing ->
    liftMaybe (BulletinUnsupportedOutputFormat $ filename) $
