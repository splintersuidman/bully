...
The Haskell program, which you can run with `cabal run bully-example`, produces the request body [`example/request.txt`](./example/request.txt).
With the server running, use
```bash
curl \
  -X GET \
  -d @example/request.txt \
